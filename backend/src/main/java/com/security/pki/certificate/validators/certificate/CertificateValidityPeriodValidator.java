...
public class CertificateValidityPeriodValidator implements CertificateValidator {
    ...
    public void validate(CertificateValidationContext context) {
        ...
        if (childCertificate.getValidFrom().after(childCertificate.getValidTo()))
            throw new CertificateValidatorException("The certificate's valid-from date cannot be after its valid-to date.");

        if (parentCertificate == null) return;

        if (childCertificate.getValidFrom().before(parentCertificate.getValidFrom()))
            throw new CertificateValidatorException("The certificate's valid-from date cannot be earlier than the signing certificate's valid-from date.");

        if (childCertificate.getValidTo().after(parentCertificate.getValidTo()))
            throw new CertificateValidatorException("The certificate's valid-to date cannot be later than the signing certificate's valid-to date.");
    }
}